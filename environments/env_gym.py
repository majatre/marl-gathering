...
class GameEnv(gym.Env):
    def __init__(self, config={}, widht=31, hight=11, agent_hidden=5, food_hidden=4):
        self.widht = widht
        self.hight = hight
        self.objects = []
        ...
        state = np.reshape(self.reset(), [-1])

        self.action_space = Discrete(8)
        self.observation_space = Box(0.0, 1.0, shape=(state.shape[0] , ))
    ...
    def step(self, agent1_action):
        self.steps_count += 1

        agent2_action = np.random.randint(8)
        ...
        if not self.agent1.is_hidden():
            agent1_action_return = self.agent1_actions[agent1_action](env_x_size=self.widht, env_y_size=self.hight)
            self.agent1_beam_set = [] if agent1_action != 6 else agent1_action_return
        if not self.agent2.is_hidden():
            agent2_action_return = self.agent2_actions[agent2_action](env_x_size=self.widht, env_y_size=self.hight)
        ...
        done = self.steps_count >= self.game_len
        return self.state(), agent1_reward, done, {}

    def contribute_metrix(self):
        a = np.ones([11 + 2, 31 + 2, 3])
        a[1:-1, 1:-1, :] = 0
    ...
    def state(self):
        progress = np.array([float(self.steps_count/self.game_len)])
        return np.reshape(self.contribute_metrix(), [-1]) #np.concatenate((progress, np.reshape(self.contribute_metrix(), [-1])))
...
class GameEnvTeam:
    def __init__(self, widht=31, hight=11, agent_hidden=5, food_hidden=4, player=1):
        self.size_x = widht
        self.size_y = hight
        self.objects = []
        self.agent_hidden = agent_hidden
